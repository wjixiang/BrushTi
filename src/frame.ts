...
export class CustomFrame {
    ...
    setInput(text) {
        // iframe 输入框的 selector
        const inputSelector = '#__next > div > div.css-1eqsjm9 > div > main > div > div > div > div.css-1y48402 > div > div.Input-sc-1w0d3vj-0.eCteNN.ProseMirror > div > div'
      
        document.querySelector(inputSelector).innerText = text
      }
