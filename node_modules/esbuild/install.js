...
function validateBinaryVersion(...command) {
  ...
  if (stdout !== "0.14.0") {
    throw new Error(`Expected ${JSON.stringify("0.14.0")} but got ${JSON.stringify(stdout)}`);
  }
}
function isYarn() {
...
function installUsingNPM(pkg, subpath, binPath) {
  ...
  try {
    ...
    child_process.execSync(`npm install --loglevel=error --prefer-offline --no-audit --progress=false ${pkg}@${"0.14.0"}`, { cwd: installDir, stdio: "pipe", env });
...
async function downloadDirectlyFromNPM(pkg, subpath, binPath) {
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg}-${"0.14.0"}.tgz`;
